...
def display_court_tab(court_type: str, get_courts_func, scrape_func):
  ...
  try:
    ...
    with col1:
      ...
      if selected_courts:
        selected_ids = [
            court['id'] for court in courts if court['name'] in selected_courts
        ]
      ...
      if st.button(f"Start Scraping {court_type} Courts"):
        ...
        try:
          with status_container.status(
            ...
            courts_data = scrape_func(conn, selected_ids)
...
with tab1:
  ...
  display_court_tab("Federal", federal_courts.scrape_federal_courts,
                    scrape_courts)

with tab2:
  st.header("State Courts")
  display_court_tab("State", state_courts.scrape_state_courts, scrape_courts)

with tab3:
  st.header("County Courts")
  display_court_tab("County", county_courts.scrape_county_courts,
                    scrape_courts)
...
with tab4:
  ...
  if schedule_enabled:
    ...
    with col2:
      start_time = st.time_input("Start Time (First Run)",
